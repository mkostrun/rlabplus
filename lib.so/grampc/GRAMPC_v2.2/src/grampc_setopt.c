...
void grampc_fprintopt(FILE *file_ptr, const typeGRAMPC *grampc)
{
  if (!file_ptr)
  {
    return;
  ...
  fprintf(file_ptr, "%s", "-- GRAMPC OPTIONS --\n");
  fprintf(file_ptr, "                         Nhor: %d\n", grampc->opt->Nhor);
  fprintf(file_ptr, "                  MaxGradIter: %d\n", grampc->opt->MaxGradIter);
  fprintf(file_ptr, "                  MaxMultIter: %d\n", grampc->opt->MaxMultIter);
  fprintf(file_ptr, "                 ShiftControl: %s\n", grampc->opt->ShiftControl == INT_ON ? "on" : "off");

  fprintf(file_ptr, "           TimeDiscretization: %s\n", grampc->opt->TimeDiscretization == INT_UNIFORM ? "uniform" : "nonuniform");

  fprintf(file_ptr, "                 IntegralCost: %s\n", grampc->opt->IntegralCost == INT_ON ? "on" : "off");
  fprintf(file_ptr, "                 TerminalCost: %s\n", grampc->opt->TerminalCost == INT_ON ? "on" : "off");
  fprintf(file_ptr, "               IntegratorCost: %s\n", grampc->opt->IntegratorCost == INT_TRAPZ ? "trapezodial" : "simpson");

  fprintf(file_ptr, "                   Integrator: %s\n", IntegratorInt2Str(grampc->opt->Integrator));
  fprintf(file_ptr, "             IntegratorRelTol: %.2e\n", grampc->opt->IntegratorRelTol);
  fprintf(file_ptr, "             IntegratorAbsTol: %.2e\n", grampc->opt->IntegratorAbsTol);
  fprintf(file_ptr, "        IntegratorMinStepSize: %.2e\n", grampc->opt->IntegratorMinStepSize);
  fprintf(file_ptr, "           IntegratorMaxSteps: %.2e\n", (typeRNum)grampc->opt->IntegratorMaxSteps);

  if (grampc->opt->Integrator == INT_RODAS)
  {
    fprintf(file_ptr, "                         IFCN: %d\n", grampc->opt->FlagsRodas[0]);
    fprintf(file_ptr, "                         IDFX: %d\n", grampc->opt->FlagsRodas[1]);
    fprintf(file_ptr, "                         IJAC: %d\n", grampc->opt->FlagsRodas[2]);
    fprintf(file_ptr, "                         IMAS: %d\n", grampc->opt->FlagsRodas[3]);
    fprintf(file_ptr, "                        MLJAC: %d\n", grampc->opt->FlagsRodas[4]);
    fprintf(file_ptr, "                        MUJAC: %d\n", grampc->opt->FlagsRodas[5]);
    fprintf(file_ptr, "                        MLMAS: %d\n", grampc->opt->FlagsRodas[6]);
    fprintf(file_ptr, "                        MUMAS: %d\n", grampc->opt->FlagsRodas[7]);
  }

  fprintf(file_ptr, "               LineSearchType: %s\n", LineSearchTypeInt2Str(grampc->opt->LineSearchType));
  fprintf(file_ptr, "    LineSearchExpAutoFallback: %s\n", grampc->opt->LineSearchExpAutoFallback == INT_ON ? "on" : "off");
  fprintf(file_ptr, "                LineSearchMax: %.2e\n", grampc->opt->LineSearchMax);
  fprintf(file_ptr, "                LineSearchMin: %.2e\n", grampc->opt->LineSearchMin);
  fprintf(file_ptr, "               LineSearchInit: %.2e\n", grampc->opt->LineSearchInit);
  fprintf(file_ptr, "        LineSearchAdaptAbsTol: %.3f\n", grampc->opt->LineSearchAdaptAbsTol);
  fprintf(file_ptr, "        LineSearchAdaptFactor: %.3f\n", grampc->opt->LineSearchAdaptFactor);
  fprintf(file_ptr, "        LineSearchIntervalTol: %.3f\n", grampc->opt->LineSearchIntervalTol);
  fprintf(file_ptr, "     LineSearchIntervalFactor: %.3f\n", grampc->opt->LineSearchIntervalFactor);

  fprintf(file_ptr, "                 OptimControl: %s\n", grampc->opt->OptimControl == INT_ON ? "on" : "off");
  fprintf(file_ptr, "                   OptimParam: %s\n", grampc->opt->OptimParam == INT_ON ? "on" : "off");
  fprintf(file_ptr, "   OptimParamLineSearchFactor: %.3f\n", grampc->opt->OptimParamLineSearchFactor);
  fprintf(file_ptr, "                    OptimTime: %s\n", grampc->opt->OptimTime == INT_ON ? "on" : "off");
  fprintf(file_ptr, "    OptimTimeLineSearchFactor: %.3f\n", grampc->opt->OptimTimeLineSearchFactor);

  fprintf(file_ptr, "                 ScaleProblem: %s\n", grampc->opt->ScaleProblem == INT_ON ? "on" : "off");
  fprint_vector(file_ptr, "                       xScale: ", grampc->opt->xScale, grampc->param->Nx);
  fprint_vector(file_ptr, "                      xOffset: ", grampc->opt->xOffset, grampc->param->Nx);
  fprint_vector(file_ptr, "                       uScale: ", grampc->opt->uScale, grampc->param->Nu);
  fprint_vector(file_ptr, "                      uOffset: ", grampc->opt->uOffset, grampc->param->Nu);
  fprint_vector(file_ptr, "                       pScale: ", grampc->opt->pScale, grampc->param->Np);
  fprint_vector(file_ptr, "                      pOffset: ", grampc->opt->pOffset, grampc->param->Np);
  fprintf(file_ptr, "                       TScale: %.3f\n", grampc->opt->TScale);
  fprintf(file_ptr, "                      TOffset: %.3f\n", grampc->opt->TOffset);
  fprintf(file_ptr, "                       JScale: %.3f\n", grampc->opt->JScale);
  fprint_vector(file_ptr, "                       cScale: ", grampc->opt->cScale, grampc->param->Nc);

  fprintf(file_ptr, "          EqualityConstraints: %s\n", grampc->opt->EqualityConstraints == INT_ON ? "on" : "off");
  fprintf(file_ptr, "        InequalityConstraints: %s\n", grampc->opt->InequalityConstraints == INT_ON ? "on" : "off");
  fprintf(file_ptr, "  TerminalEqualityConstraints: %s\n", grampc->opt->TerminalEqualityConstraints == INT_ON ? "on" : "off");
  fprintf(file_ptr, "TerminalInequalityConstraints: %s\n", grampc->opt->TerminalInequalityConstraints == INT_ON ? "on" : "off");
  fprintf(file_ptr, "          ConstraintsHandling: %s\n", grampc->opt->ConstraintsHandling == INT_EXTPEN ? "extpen" : "auglag");
  fprint_vector(file_ptr, "             ConstraintAbsTol: ", grampc->opt->ConstraintsAbsTol, grampc->param->Nc);

  fprintf(file_ptr, "                MultiplierMax: %.2e\n", grampc->opt->MultiplierMax);
  fprintf(file_ptr, "      MultiplierDampingFactor: %.3f\n", grampc->opt->MultiplierDampingFactor);
  fprintf(file_ptr, "                   PenaltyMax: %.2e\n", grampc->opt->PenaltyMax);
  fprintf(file_ptr, "                   PenaltyMin: %.2e\n", grampc->opt->PenaltyMin);
  fprintf(file_ptr, "        PenaltyIncreaseFactor: %.3f\n", grampc->opt->PenaltyIncreaseFactor);
  fprintf(file_ptr, "        PenaltyDecreaseFactor: %.3f\n", grampc->opt->PenaltyDecreaseFactor);
  fprintf(file_ptr, "     PenaltyIncreaseThreshold: %.3f\n", grampc->opt->PenaltyIncreaseThreshold);
  fprintf(file_ptr, "   AugLagUpdateGradientRelTol: %.2e\n", grampc->opt->AugLagUpdateGradientRelTol);

  fprintf(file_ptr, "             ConvergenceCheck: %s\n", grampc->opt->ConvergenceCheck == INT_ON ? "on" : "off");
  fprintf(file_ptr, "    ConvergenceGradientRelTol: %.2e\n", grampc->opt->ConvergenceGradientRelTol);
