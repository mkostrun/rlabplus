...
void fprint_vector(FILE *f, const char *prefix, ctypeRNum *vector, ctypeInt size)
{
  typeInt i;
  if (vector == NULL)
  {
...
typeInt fprint_singleStatus(FILE *f, ctypeInt status, ctypeInt statusmask, const typeChar*message)
{
...
typeInt grampc_fprintstatus(FILE *f, ctypeInt status, ctypeInt level)
{
  typeInt printed = 0;
  if (level & 1)
  {
    printed |= fprint_singleStatus(f, status, STATUS_INTEGRATOR_INPUT_NOT_CONSISTENT, STATUS_MSG_INTEGRATOR_INPUT_NOT_CONSISTENT);
    printed |= fprint_singleStatus(f, status, STATUS_INTEGRATOR_MAXSTEPS, STATUS_MSG_INTEGRATOR_MAXSTEPS);
    printed |= fprint_singleStatus(f, status, STATUS_INTEGRATOR_STEPS_TOO_SMALL, STATUS_MSG_INTEGRATOR_STEPS_TOO_SMALL);
    printed |= fprint_singleStatus(f, status, STATUS_INTEGRATOR_MATRIX_IS_SINGULAR, STATUS_MSG_INTEGRATOR_MATRIX_IS_SINGULAR);
    printed |= fprint_singleStatus(f, status, STATUS_INTEGRATOR_H_MIN, STATUS_MSG_INTEGRATOR_H_MIN);
  }
  if (level & 2)
  {
    printed |= fprint_singleStatus(f, status, STATUS_MULTIPLIER_MAX, STATUS_MSG_MULTIPLIER_MAX);
    printed |= fprint_singleStatus(f, status, STATUS_PENALTY_MAX, STATUS_MSG_PENALTY_MAX);
    printed |= fprint_singleStatus(f, status, STATUS_INFEASIBLE, STATUS_MSG_INFEASIBLE);
  }
  if (level & 4)
  {
    printed |= fprint_singleStatus(f, status, STATUS_GRADIENT_CONVERGED, STATUS_MSG_GRADIENT_CONVERGED);
    printed |= fprint_singleStatus(f, status, STATUS_CONSTRAINTS_CONVERGED, STATUS_MSG_CONSTRAINTS_CONVERGED);
    printed |= fprint_singleStatus(f, status, STATUS_LINESEARCH_INIT, STATUS_MSG_LINESEARCH_INIT);
  }
  if (level & 8)
  {
    printed |= fprint_singleStatus(f, status, STATUS_LINESEARCH_MIN, STATUS_MSG_LINESEARCH_MIN);
    printed |= fprint_singleStatus(f, status, STATUS_LINESEARCH_MAX, STATUS_MSG_LINESEARCH_MAX);
    printed |= fprint_singleStatus(f, status, STATUS_MULTIPLIER_UPDATE, STATUS_MSG_MULTIPLIER_UPDATE);
  }
  return printed;
}
