...
#define ABS_ZERO 1e-6
#define REL_ZERO 1e-3
...
//   fprintf(stdout, "point_locate: prev=%p, on_summit=%i\n",prev,on_summit);
  ...
  if (!prev)
  {
    ...
    fprintf(stdout, "point_locate:1 NULL\n");
    return NULL;
  }

  f = neighbor (f, prev, surface);

//   fprintf(stdout, "point_locate: neighbor: f=%p\n",f);
...
//       fprintf(stdout, "point_locate:2 NULL\n");
      return NULL;
    }
...
//   fprintf(stdout, "point_locate:3 NULL\n");
  return NULL;
}
			    ...
			    GtsFace * guess)
{
  ...
  g_return_val_if_fail (guess == NULL || gts_face_has_parent_surface (guess, surface), NULL);
//   fprintf(stdout, "gts_point_locate: passed initial checks without fail\n");
  ...
  {
//     fprintf(stdout, "gts_point_locate: gts_triangle_orientation: FAIL\n");
    g_return_val_if_fail (gts_triangle_orientation (GTS_TRIANGLE (guess)) > 0., NULL);
  }

  if (guess == NULL)
  {
//     fprintf(stdout, "gts_point_locate: closest_face: NULL\n");
    return NULL;
  }

//   fprintf(stdout, "gts_point_locate: guess=%p\n", guess);
...
//   fprintf(stdout, "gts_point_locate: return fr=%p\n", fr);
					     ...
					     GtsFace * f)
{
  ...
  if (GTS_IS_LIST_FACE (f) && GTS_IS_LIST_FACE (nf[0]))
  {
    ...
    while (i)
    {
      ...
      if (p != GTS_POINT (v))
      {
        if (gts_point_orientation (GTS_POINT (v), GTS_POINT (v1), p) >= ABS_ZERO)
        {
...
//                  > 0. ? 0 : 1;
           ...
           {
             o = gts_point_orientation (GTS_POINT (v), GTS_POINT (v3), p);
             j = o > ABS_ZERO ? 0 : 1;
           }
        }
        else if (gts_point_orientation (GTS_POINT (v), GTS_POINT (v3), p) > ABS_ZERO)
          j = 2;
        else
          j = 1;
...
GtsVertex * gts_delaunay_add_vertex_to_face_old (GtsSurface * surface,
    GtsVertex * v,
    GtsFace * f)
{
  GtsEdge * e1, * e2, * e3;
